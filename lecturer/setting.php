...
$select = "SELECT * FROM person WHERE personId = '$lecturerId' AND personPassword = '$old'";
$sql_query = mysqli_query($conn, $select);
$count = mysqli_num_rows($sql_query);
        if ($count > 0){         
        $update_query = "UPDATE `person` SET `personPassword` = '$new' WHERE `person`.`personId` = '$lecturerId'";
